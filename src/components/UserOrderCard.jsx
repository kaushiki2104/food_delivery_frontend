...
function UserOrderCard({ data }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 space-y-4">
      <div className="flex justify-between border-b pb-2">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Date:{formetDate(data.createdAt)}
          </p>
        </div>
        <div className="text-right">
          {data.paymentMethod=="cod"?  <p className="text-sm text-gray-500">
            {data.paymentMethod?.toUpperCase()}
          </p>: <p className="text-sm text-gray-500 font-semibold">Payment:
          ...
          <div className="flex space-x-4 overflow-x-auto pb-2">
            {shopOrder.shopOrderItems.map((item, index) => (
              ...
              >
                ...
                />
                <p text-small font-semibold mt-1>
                  {item.name}
                </p>
      ...
      <div className="flex justify-between items-center border-t pt-2">
        <p className="font-semobold">Total: ₹{data.totalAmount}</p>
