  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 space-y-4">
      <div>
        ...
        {data.paymentMethod=="online"?<p className="gap-2 text-sm text-gray-600">payment:{data.payment?"true":"false"}</
      ...
      <div className="flex space-x-4 overflow-x-auto pb-2">
        {(data.shopOrders?.shopOrderItems || []).map((item, index) => (
          ...
          >
            ...
            />
            <p className="text-small font-semibold mt-1">{item.name}</p>
            <p className="text-xs text-gray-500">
      ...
      {data.shopOrders?.status === "out of delivery" && 
        <div className="mt-3 p-2 border rounded-lg text-sm bg-orange-50">      
          ...
          ) :data.shopOrders.assignedDeliveryBoy?<div>{data.shopOrders.assignedDeliveryBoy.
          fullName}</div> :<div>Waiting for delivery boy to accept</div>}
      </div>}
