  ...
  const verifyOtp=async ()=>{
    try{
        const result = await axios.post(`${serverUrl}/api/order/verify-delivery-otp`,{
          shopOrderId:currentOrder.shopOrder._id, otp
        },
        { withCredentials: true }
      )
      // setAvailableAssignments(result.data)
  ...
  return (
    <div className='w-screen min-h-screen flex flex-col gap-5 items-center bg-[#fff9f6] overflow-y-auto'>
      ...
      <div className='w-full max-w-[800px] flex flex-col gap-5 items-center'>
        ...
        justify-start
         items-center w-[90%] border-orange-100 text-center gap-2'>
     ...
     border-orange-100'>
        ...
        border-orange-100'>
          ...
          <div className='border rounded-lg p-4 mb-3'>
            ...
            <p className='text-xs text-gray-400'>{currentOrder.shopOrder.shopOrderItems.length}
              items | {currentOrder.shopOrder.subtotal}
            </p>
          </div>
          <DeliveryBoyTracking data={currentOrder}/>
          {!showOtpBox ?  <button className='mt-4 w-full bg-green-500 text-white font-semibold py-2 px-4
          rounded-xl shadow-md hover:bg-green-600 active:scale-95 transition-all
          duration-200'onClick={sendOtp}> 
            Mark As Delivered 
            ...
            <p className='text-sm font-semibold mb-2'>Enter Otp send to <span className='text-orange-500'>
              ...
              focus:ring-orange-400' placeholder='Enter OTP' onChange={(e)=>sendOtp(e.target.value)} value={otp}/>
